...
auto Utils::StringToLower(const std::string_view s) -> std::string {
    std::string tmp{s};
    std::ranges::transform(tmp, tmp.begin(), [](unsigned char c) { return std::tolower(c); });
    return tmp;
}

auto Utils::StringToUpper(const std::string_view s) -> std::string {
    std::string tmp{s};
    std::ranges::transform(tmp, tmp.begin(), [](unsigned char c) { return std::toupper(c); });
    return tmp;
}

auto Utils::TrimLeft(const std::string_view s) -> std::string {
    std::string tmp{s};
    ltrim(tmp);
    return tmp;
}

auto Utils::TrimRight(const std::string_view s) -> std::string {
    std::string tmp{s};
    rtrim(tmp);
    return tmp;
}

auto Utils::Trim(const std::string_view s) -> std::string {
    std::string tmp{s};
    trim(tmp);
    return tmp;
}

auto Utils::StringCaseCmp(std::string_view a, std::string_view b) -> bool {
    return a.size() == b.size() && std::ranges::equal(a, b, [](char c1, char c2) { return std::tolower(c1) == std::tolower(c2); });
}
...
auto Utils::FixIndent(const std::string_view s) -> std::string {
    ...
    while (std::getline(ss, line)) {
        ...
        if (currentIndent == std::string::npos) {
            ...
            // If the line is shorter than minIndent (shouldn't happen with
            // correct logic, but substr handles it gracefully), or just remove
            // the common indent.
            output << line.substr(minIndent);
        }
    }
    // Trim any leading or trailing newlines that might have resulted from
    // empty lines at the start or end of the input (not strictly required by
    // the prompt, but often desired for clean output).
