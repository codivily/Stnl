...
class DB {
  ...
  public:
    ...
    std::future<QResult> QInsert(std::string tableName, Args &&...columnValuePairs) {
        return Utils::AsFuture<QResult>(ioc_, [this, tableName = std::move(tableName),
                                               columnValuePairs = std::make_tuple(std::forward<Args>(columnValuePairs)...)]() mutable {
            return std::apply([this, tableName = std::move(tableName)](auto &&...vals) { return Insert<S>(tableName, std::forward<decltype(vals)>(vals)...); },
                              columnValuePairs);
        });
    }

    QResult InsertBatch(std::string const &tableName, const std::function<void(BatchInserter &batch)> &populateBatchFn);
    std::future<QResult> QInsertBatch(std::string const &tableName, std::function<void(BatchInserter &batch)> populateBatchFn);

    void Work(const std::function<void(pqxx::work &tx)> &doWorkFn);
    std::future<void> QWork(std::function<void(pqxx::work &tx)> doWork);

    Migration &GetMigration();

    static std::string GetConnectionString(std::string_view dbName, std::string_view dbUser, std::string_view dbPassword, std::string_view dbHost = "localhost",
                                           size_t dbPort = 5432, std::string_view dbSchema = "public");
