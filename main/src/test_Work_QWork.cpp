...
int main(int argc, char argv[]) {
    ...
    // std::unordered_map<size_t, std::string> const& dataTypes =
    // db.GetDataTypes(); for(const auto& [oid, typname] : dataTypes) {
    ...
    //     for (auto [uuid, name, utcdt] : tx.stream<std::string_view,
    //     std::string_view, std::string_view>("SELECT uuid, name, utcdt FROM
    //     product LIMIT 10")) {
    //       Logger::Dbg() << std::format("UUID: {}, name: {}, utcdt: {}", uuid,
    //       name, utcdt);
    //     }
    //     tx.commit();
    // });
