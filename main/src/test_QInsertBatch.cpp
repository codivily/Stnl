...
int main(int argc, char argv[]) {
    ...
    // options=-csearch_path=stnl_sch,public"); Migrator migrator;
    ...
    // QResult r = db.QExec("SELECT id, uuid, name, description, utcdt, active
    // FROM product ORDER BY utcdt ASC LIMIT 15").get(); try {
    //   for(const pqxx::row& row : r.data) {
    //     auto [id, uuid, name, description, utcdt, active] = row.as<long long,
    //     char const*, char const*, char const*, char const*, bool>();
    //     Logger::Dbg() << std::format("{} - {} - {} - {} - {} - {}", id, uuid,
    //     name, (description == nullptr ? "<null>" : description), utcdt,
    //     active);
    //   }
    ...
    std::future<QResult> fut = db.QInsertBatch("Product", [](BatchInserter &batch) {
        ...
        size_t i = 0;
        while (true) {
            batch << std::make_pair("name", "Green-Qeen - " + std::to_string(i)) << std::make_pair("description", "Gk") << std::make_pair("price", 12.9)
